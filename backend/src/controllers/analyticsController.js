...
export const createDashboard = async (req, res) => {
  try {
    const { name } = req.body;
    const dashboard = await prisma.dashboard.create({ data: { name } });
    res.json(dashboard);
  } catch (err) {
    res.status(500).json({ error: "Failed to create dashboard", details: err.message });
  }
...
// Panels
export const getPanelsByDashboard = async (req, res) => {
  try {
    const { id } = req.params;
    const panels = await prisma.panel.findMany({
      where: { dashboardId: parseInt(id) }
    });
    res.json(panels);
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch panels", details: err.message });
...
export const createPanel = async (req, res) => {
  try {
    const { dashboardId, title, promql, chartType, thresholds, layout, visualizationConfig } = req.body;
    const panel = await prisma.panel.create({
      data: {
        dashboardId,
        title,
